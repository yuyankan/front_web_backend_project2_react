...
const ImageDisplayArea: React.FC = () => {
    ...
    const activeKey = useMemo(() => {
        //change to do not show pictures all the time
        //return selectedPoint?.image_url || latestinfodata?.image_url;
        return selectedPoint?.image_url;
    ...
    return (
        <>
            <h3 style={{fontSize:'30px', position: 'relative', color: 'black', padding: '0px 0px 0px 30px',}}>
                IMAGE INSPECTION: 
            </h3>
               
            <div className='right_panel_image_sub'>
                <div className="image_wrapper" >
                    
                    {/* 状态提示：历史记录模式 */}
                    {selectedPoint && (
                        <div style={{ position: 'relative', top: 0, left: 0, zIndex: 10, background: 'rgba(252, 7, 7, 0.8)', color: '#fff', padding: '4px 12px', fontSize: '18px' }}>
                            HISTORY MODE (Auto-return in 8s)
                        </div>
                    )}
                ...
                <div className="image_detail_panel">
                    <h4 style={{ borderBottom: '1px solid #ddd', padding: '0px',fontSize:'15px' }}>{statusLabel}:</h4>
                    {displayData ? (
                        <ul style={{ listStyle: 'none', padding: 0 }}>
                            {allowedKeys.map((key) => {
                                ...
                                return (
                                    <li key={key} style={{ display: 'flex', alignItems: 'baseline', marginBottom: '12px', fontSize: '15px' }}>
                                        <strong style={{ color: '#666', minWidth: '100px', fontSize: '15px' }}>
